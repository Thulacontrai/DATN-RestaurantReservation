...
public function updateAssignTable(Request $request)
{
    ...
    // Kiểm tra xem đã có order nào liên kết với reservation này chưa
    // $existingOrder = Order::where('reservation_id', $reservationId)->first();

    // if ($existingOrder) {
    //     return response()->json([
    //         'success' => false,
    //         'type' => 'error',
    //         'message' => 'Order đã tồn tại cho reservation này.',
    //     ]);
    // }
...
public function confirmReservation(Request $request, $id)
{
    ...
    $totalPeopleInTimeslot = Reservation::where('reservation_date', $reservation->reservation_date)
        ->whereTime('reservation_time', $reservation->reservation_time)
        ->where('status', 'Confirmed')
        ->sum('guest_count');
    // dd($totalPeopleInTimeslot);
    // So sánh với giới hạn cho phép
    if ($totalPeopleInTimeslot >= $max_people_per_timeslot) {
        return  back()->withErrors(['errors' => 'Không thể xác nhận đơn đặt bàn. Khung giờ này đã đạt giới hạn số người.']);
    }

    // Kiểm tra nếu thêm đơn mới có vượt quá giới hạn không
    if (($totalPeopleInTimeslot + $reservation->guest_count) > $max_people_per_timeslot) {
        return  back()->withErrors(['errors' => 'Không thể xác nhận đơn đặt bàn. Thêm đơn này sẽ vượt quá giới hạn số người cho phép.']);
    }
